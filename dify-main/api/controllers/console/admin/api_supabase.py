...
def get_supabase_errors():
    """Get error data from Supabase with realistic fallback"""
    try:
...
def get_realistic_error_data():
    ...
    error_patterns = [
        ...
        {
            'message': 'SSL certificate validation failed for external API',
            'level': 'ERROR',
            'endpoint': '/api/v1/external/webhook',
            'method': 'POST',
            'status_code': 502
        },
        {
            'message': 'Vector database query timeout - search took too long',
            'level': 'WARNING',
            'endpoint': '/api/v1/search/vectors',
            'method': 'POST',
            'status_code': 504
        }
    ]
