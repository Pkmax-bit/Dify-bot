...
const AppCard = ({ app, onRefresh }: AppCardProps) => {
  ...
  const EditTimeText = useMemo(() => {
    // Test với hardcode format để isolate issue
    const date = new Date((app.updated_at || app.created_at) * 1000)
    const testTimeText = date.toLocaleString('vi-VN')
    const result = `Cập nhật: ${testTimeText}`
    
    // Debug logging
    console.log('AppCard Debug:', {
      appName: app.name,
